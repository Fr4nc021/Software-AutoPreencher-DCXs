        ...
        for modelo in modelos_selecionados:
            ...
            for paragrafo in doc.paragraphs:
                for chave, valor in dados.items():
                    for paragrafo in doc.paragraphs:
    if paragrafo.runs:  # Garante que o parágrafo tenha texto
        texto_completo = "".join(run.text for run in paragrafo.runs)  # Junta todo o texto do parágrafo

        for chave, valor in dados.items():
            texto_completo = texto_completo.replace(f"{{{{{chave}}}}}", valor)  # Substitui os placeholders

        if texto_completo != "".join(run.text for run in paragrafo.runs):  # Verifica se houve alteração
            paragrafo.clear()  # Limpa o parágrafo mantendo a formatação
            novo_run = paragrafo.add_run(texto_completo)  # Adiciona o texto substituído

            # Mantém a formatação original se houver runs no parágrafo
            if paragrafo.runs:
                novo_run.font.name = paragrafo.runs[0].font.name
                novo_run.font.size = paragrafo.runs[0].font.size
