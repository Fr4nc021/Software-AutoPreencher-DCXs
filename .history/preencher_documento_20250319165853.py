...
tela.geometry("400x600")  # Janela mais estreita

ctk.CTkLabel(tela, text="📄 Gerador de Documentos", font=("Arial", 18, "bold")).pack(pady=10)

modelo_checkboxes = {}
frame_check = ctk.CTkFrame(tela)
frame_check.pack(pady=10, padx=20, fill="x", expand=True)  # Margens laterais
for modelo_id, modelo_nome in modelos_disponiveis.items():
    var = ctk.BooleanVar()
    ctk.CTkCheckBox(frame_check, text=os.path.basename(modelo_nome), variable=var).pack(anchor="w", fill="x", padx=5)

frame_campos = ctk.CTkFrame(tela)
frame_campos.pack(pady=10, padx=20, fill="x", expand=True)  # Margens laterais
...
for label_texto, chave in campo_labels:
    ...
    entrada = ctk.CTkEntry(frame, width=200)  # Ajustado para ser mais estreito
...
frame_botoes.pack(pady=10, padx=20, fill="x", expand=True)  # Margens laterais
ctk.CTkButton(frame_botoes, text="Gerar Selecionados", command=coletar_dados, width=150).pack(side="left", padx=5)
ctk.CTkButton(frame_botoes, text="Gerar Todos", command=selecionar_e_gerar, width=150).pack(side="right", padx=5)
