...
pasta_modelos = os.path.dirname(__file__)  # Obtém o diretório do script atual
modelos_disponiveis = {
    "1": "1.ACERVO.docx",
    "2": "2.DSA.docx",
    "3": "3.IDONEIDADE.docx",
...
# Criar interface gráfica com Tkinter (usando ttk para um design melhor)
root = tk.Tk()
root.title("Gerador de Documentos")
root.geometry("500x550")  # Tamanho da janela
root.configure(bg="#f4f4f4")  # Cor de fundo

from tkinter import ttk  # Importação correta do ttk

style = ttk.Style()  # Criar estilo corretamente
...
label_modelo = tk.Label(frame_modelo, text="📌 Escolha o tipo de documento:", font=("Arial", 11, "bold"), bg="#f4f4f4")
label_modelo.pack()

modelo_var = tk.StringVar(root)
modelo_var.set("Declaração de Acervo")  # Define um modelo padrão

menu_modelo = tk.ttk.Combobox(frame_modelo, textvariable=modelo_var, values=list(modelos_disponiveis.keys()), state="readonly")
...
btn_gerar_todos = tk.Button(btn_frame, text="Gerar Todos os Documentos", command=selecionar_e_gerar)
