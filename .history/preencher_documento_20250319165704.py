...
frame_check.pack(pady=10, fill="x", expand=True)  # Tornando o frame expansível
for modelo_id, modelo_nome in modelos_disponiveis.items():
    var = ctk.BooleanVar()
    ctk.CTkCheckBox(frame_check, text=os.path.basename(modelo_nome), variable=var).pack(anchor="w", fill="x")
    modelo_checkboxes[modelo_id] = var

frame_campos = ctk.CTkFrame(tela)
frame_campos.pack(pady=10, fill="x", expand=True)  # Tornando o frame expansível
...
frame_botoes.pack(pady=10, fill="x", expand=True)  # Tornando o frame expansível
