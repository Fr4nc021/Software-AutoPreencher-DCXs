...
for modelo_id, modelo_nome in modelos_disponiveis.items():
    var = tk.BooleanVar()
    check = tk.Checkbutton(root, text=modelo_nome, variable=var)
    check.grid(row=int(modelo_id), column=0, sticky="w", padx=10, pady=5)
...
for i, (label_texto, chave) in enumerate(campos):
    ...
    label.grid(row=i + len(modelos_disponiveis), column=0, sticky="w", padx=10, pady=5)
    entry = tk.Entry(root)
    entry.grid(row=i + len(modelos_disponiveis), column=1, padx=10, pady=5)
...
# Criar botões
btn_gerar = tk.Button(root, text="Gerar Documentos Selecionados", command=coletar_dados)
btn_gerar.grid(row=len(campos) + len(modelos_disponiveis), column=0, columnspan=2, pady=10)

btn_gerar_todos = tk.Button(root, text="Gerar Todos os Documentos", command=selecionar_todos)
btn_gerar_todos.grid(row=len(campos) + len(modelos_disponiveis) + 1, column=0, columnspan=2, pady=10)
